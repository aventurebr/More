        ...
        <div className="max-w-3xl">
          ...
          <FadeIn delay={300}>
            <div className="flex flex-col sm:flex-row gap-2">
              <Input type="text" placeholder="Digite um bairro ou universidade..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="h-12 bg-white rounded-md focus-visible:ring-0 text-base flex-1 py-[8px]" />
              <Button type="submit" className="h-12 px-6 bg-slate-700 hover:bg-slate-800">
          ...
          <FadeIn delay={400}>
            <div className="flex flex-wrap gap-2 mt-6">
              {["Centro", "Barão Geraldo", "Próximo à Unicamp", "Taquaral"].map((location, index) => <Button key={index} variant="secondary" className="text-white rounded-full border-0 bg-[#484848]/[0.57]">
                  {location}
                </Button>)}
            </div>
          </FadeIn>
